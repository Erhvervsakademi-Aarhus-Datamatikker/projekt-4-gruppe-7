...
public class Controller {
    ...
    public static Bestilling opretBestillingMedPladser(Forestilling forestilling,
                                                       Kunde kunde, LocalDate dato, ArrayList<Plads> pladser) {
        for (Plads plads : pladser) {
            if (!forestilling.erPladsLedig(plads.getRække(), plads.getNr(), dato)
                    || !dato.isAfter(forestilling.getStartDato()) || !dato.isBefore(forestilling.getSlutDato())) {
                return null;
            }
        ...
        for (Plads plads : pladser) {
            bestilling.addBestiltePladser(plads);
        }
